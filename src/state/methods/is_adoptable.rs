...
impl State {
    ...
    ///
    /// Rejecting unrepresentable readings keeps [`State::last_ts`] within the
    /// range the time-based constructors accept, so one bogus reading from a
    /// broken clock cannot permanently poison the shared state.
