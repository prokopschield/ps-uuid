...
impl UUID {
    ...
    #[must_use]
    pub fn new_v7(timestamp: Duration, random_bytes: [u8; 7]) -> Self {
        ...
        // Byte 8: Variant (0x80) + 6 timestamp bits
        uuid.bytes[8] = 0x80 | (nanos[3] >> 2);

        // The last 2 bits of timestamp nanoseconds are discarded.
        // The precision of timestamps is up to 4 nanoseconds.

        // Caller-provided random payload
        uuid.bytes[9..].copy_from_slice(&random_bytes);
...
mod tests {
    ...
    fn version_and_variant_are_correct() {
        let uuid = UUID::new_v7(Duration::from_nanos(0), [0; 7]);
    ...
    fn different_random_payloads_differ() {
        ...
        let a = UUID::new_v7(ts, [0; 7]);
        let b = UUID::new_v7(ts, [1; 7]);
