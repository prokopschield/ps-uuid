...
impl UUID {
    ...
    ///   epoch: a fresh state starts at the UUID epoch, the `FILETIME` epoch,
    ///   or the Unix epoch, whichever is the earliest the platform clock can
    ///   represent, so on platforms whose clock reaches back to 1582-10-15
    ///   the error persists until a reading at or after 1601-01-01 is
    ///   adopted, and on the other platforms it never arises from a fresh
    ///   state.
    ///
